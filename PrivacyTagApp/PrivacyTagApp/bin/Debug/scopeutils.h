...
#pragma warning(suppress: 4100)
    ...
    public:
		UniverseSampler(double prob, unsigned long seed) : m_sampleProb(prob), m_seed(seed), m_useDivisor(true)
        {
			...
			m_divisor = (unsigned __int64) floor(4.0 / prob);
        ...
        double PickRow(unsigned __int64 hash_value)
        {
            ...
            unsigned __int64 sv3 = (v1 >> 1) % m_divisor;
            unsigned __int64 sv4 = (v2 >> 5) % m_divisor;
			...
			bool picked =
				...
				(m_sampleProb > 0.5 && (halfv1 == 0 || halfv2 == 0)) || /* the over-half case [sk: we have to give 1-of-two chance; each halfv is uniform over [0,7]] */
				(m_useDivisor && (sv1 == 0 || sv2 == 0 || sv3 == 0 || sv4 == 0)) /* the residual [sk: we have to give 4-in-divisor chances...]*/
				);                
